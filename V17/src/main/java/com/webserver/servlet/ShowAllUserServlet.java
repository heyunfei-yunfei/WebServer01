...
public class ShowAllUserServlet extends HttpServlet{
    ...
    public void service(HttpRequest request, HttpResponse response){
     ...
     //1 从user。dat文件中将所有用户信息读取出来
        ...
        //将user.dat文件用户读取后以User对象存入list集合
        ...
        //2使用thymeleaf将所有用户信息与userlist.html页面结合页面并生成动态页面
        //2.1所有要在页面上显示的动态数据都需要存入一个名为Context的类中
        ...
        /*
        String process(String trmplate , Context context)
        该方法就是thymeleaf将模板与context中的数据进行结合来生成一个动态页面，返回值就是
        生成后的动态页面的源代码（html代码）
         */
