...
public class ReServlet {
    public void service(HttpRequest request, HttpResponse response){
        ...
        System.out.println(username+","+password+","+nickname+","+ageStr);
          ...
          /*添加对用户输入的信息的验证工作
          要求：
          如果用户名，密码，昵称，和年龄为null，或者年龄输入的不是一个数字（正则表达式验证）
          则直接设置response响应一个注册失败的提示页面。
          该页面凡在webapps/myweb目录下，名字为reg_input_error.html
          页面中居中显示一行字：注册信息输入有误请重新注册
          然后加一个超链接返回到注册页面
        ...
        try(RandomAccessFile raf = new RandomAccessFile("user.dat","rw");
            ...
            /*先读取user.dat文件中现有的所有数据，将每条记录的用户读取出来并与当前注册用户的
            用户名对比，如果已经存在，则直接响应页面：have_user.html.
            该页面居中显示一行字:该用户存在，请重新注册。
